...
MS_SCOPES = os.environ.get("MS_SCOPES",
    "offline_access Files.ReadWrite Mail.Send User.Read").split()
...
MS_CONNECTION_DOC_ID = "ms_connection"  # singleton in db.integrations
...
async def _ms_save_tokens(token_result: dict, user_email: Optional[str] = None,
                          extra: Optional[dict] = None) -> None:
    doc = await db.integrations.find_one({"_id": MS_CONNECTION_DOC_ID}) or {}
    update = {
        "_id": MS_CONNECTION_DOC_ID,
        "provider": "microsoft",
        "refresh_token": token_result.get("refresh_token") or doc.get("refresh_token"),
        "scope": token_result.get("scope", doc.get("scope")),
        "updated_at": now_iso(),
        "connected_email": user_email or doc.get("connected_email"),
    }
    if extra:
        update.update(extra)
...
async def _ms_get_access_token() -> Optional[str]:
    ...
    if not doc or not doc.get("refresh_token"):
        return None
    msal_app = _ms_msal_app()
    res = msal_app.acquire_token_by_refresh_token(
        doc["refresh_token"], scopes=MS_SCOPES)
    if "access_token" not in res:
        logger.error(f"MS refresh failed: {res.get('error_description', res)}")
        return None
    # Store rotated refresh token if Microsoft returned a new one
    if res.get("refresh_token"):
        await _ms_save_tokens(res)
    return res["access_token"]


async def _ms_upload_to_onedrive(access_token: str, folder: str,
                                 filename: str, content: bytes) -> dict:
    """Upload a binary file to /me/drive/root:/{folder}/{filename}: using an
    upload session (works for files larger than 4 MB)."""
    ...
    async with _httpx.AsyncClient(timeout=120) as cx:
        ...
        while i < total:
            ...
            if r.status_code not in (200, 201, 202):
                raise RuntimeError(f"OneDrive chunk failed {r.status_code}: {r.text}")
...
async def _ms_send_outlook_mail(access_token: str, to: List[str], subject: str,
                                body_html: str,
                                attachment: Optional[dict] = None) -> None:
    """Send Outlook mail. `attachment` = {name, content_b64} (small only). For
    a 28MB attachment we instead embed a OneDrive share link in the body."""
    msg: dict = {
        "subject": subject,
    ...
    async with _httpx.AsyncClient(timeout=60) as cx:
        r = await cx.post(
        ...
        if r.status_code not in (200, 202):
            raise RuntimeError(f"Outlook sendMail failed: {r.status_code} {r.text}")


async def _ms_create_share_link(access_token: str, item_id: str) -> Optional[str]:
    async with _httpx.AsyncClient(timeout=30) as cx:
        r = await cx.post(
...
async def _ms_run_monthly_export() -> dict:
    """Build current audit binder PDF, upload to OneDrive, optionally email."""
    doc = await db.integrations.find_one({"_id": MS_CONNECTION_DOC_ID})
    if not doc or not doc.get("refresh_token"):
    ...
    item = await _ms_upload_to_onedrive(access, MS_BINDER_FOLDER, filename, pdf_bytes)
    ...
    # Optional email to a recipient list (configured via db doc.email_to)
...
class MsEmailReq(BaseModel):
    email_to: Optional[str] = None  # comma-separated list, or empty to clear


@api.get("/ms/status")
async def ms_status(current: UserPublic = Depends(require_admin)):
    doc = await db.integrations.find_one(
        {"_id": MS_CONNECTION_DOC_ID}, {"_id": 0, "refresh_token": 0}) or {}
    return {
        ...
        "schedule": "Runs on the 1st of every month at 02:00 " + MS_BINDER_TZ,
    }
...
async def ms_auth_url(current: UserPublic = Depends(require_admin)):
    if not _ms_configured():
        raise HTTPException(503, "Microsoft integration not configured on server.")
    msal_app = _ms_msal_app()
...
@api.get("/ms/callback")
async def ms_callback(code: Optional[str] = None,
                      error: Optional[str] = None,
                      error_description: Optional[str] = None,
                      state: Optional[str] = None):
    """OAuth redirect handler. Returns a self-contained HTML page so the
    admin can close the tab (mobile or web)."""
...
<a class='btn' href='/'>Return to app</a></div></body></html>"""
    ...
    if error:
        return _html_page(False, "Microsoft sign-in cancelled",
                          error_description or error)
    if not code:
        return _html_page(False, "Missing code",
    ...
    try:
        ...
        if "access_token" not in result:
            return _html_page(False, "Token exchange failed",
        ...
        return _html_page(True, "Microsoft 365 Connected",
...
@api.post("/ms/email-recipients")
async def ms_email_recipients(req: MsEmailReq,
                              current: UserPublic = Depends(require_admin)):
    await db.integrations.update_one(
        {"_id": MS_CONNECTION_DOC_ID},
        {"$set": {"email_to": (req.email_to or "").strip(), "updated_at": now_iso()}},
        upsert=True,
    )
    return {"ok": True}


@api.post("/ms/export-now")
...
# ---------- SCHEDULER (monthly binder export) ----------
...
async def _start_ms_scheduler():
    ...
    try:
        ...
        _SCHEDULER.start()
        logger.info("MS Graph monthly export scheduler started (1st of month, 02:00 %s)",
                    MS_BINDER_TZ)
    except Exception as e:
